...
#Include for force torque sensor
...
#        self.armPose = PoseStamped()
        ...
        rospy.Subscriber("/valve_tracker/valve",
                         PoseWithCovarianceStamped,
                         self.updateGoalPose,
                         queue_size = 1)
        ...
        self.pub_decision = rospy.Publisher('/rfdm/reactive', rfdm_msg)
...
#        self.arm_pose_init = False
    ...
    def updateForceTorque(self, wrench_msg):
        ...
        try:
            self.force_old = self.force_new
            self.force_new = wrench_msg.wrench.force.z
        finally:
    ...
    def evaluatePosition(self):
        ...
        # rospy.loginfo('Distance ' + str(self.distance_goal))
        # rospy.loginfo('Old force ' + str(self.force_old) + ' New force ' + str(self.force_new))
        if ( self.distance_goal >= 0.8  ):
#        if ( self.distance_goal >= 0.25  ):
            if self.going_backward:
                rospy.loginfo('Going backward')
                if (np.abs(rospy.get_time() - self.init_time_backward) < 30.0):
                    return -1.0
                else:
                    self.going_backward = False
                    return 1.0
            else:
                if np.abs(self.force_new - self.force_old) >= 5.0:
                    self.init_time_backward = rospy.get_time()
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            if self.initRobotPose and self.initGoal :
                ...
                # rospy.loginfo('Safe_rate ' + str(safe_rate))
                # rospy.loginfo('******************************************')
                if safe_rate == 1.0:
...
if __name__ == '__main__':
    try:
        ...
        config_file_list = roslib.packages.find_resource(
            "udg_pandora_misc", "work_area_v3.yaml")
        if len(config_file_list):
            config_file = config_file_list[0]
            subprocess.call(["rosparam", "load", config_file])
        else:
            rospy.logerr("Could not locate work_area_v3.yaml")

        rospy.init_node('work_area_v3')

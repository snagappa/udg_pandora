...
#        self.publishJoyMessage()
    ...
    def publishJoyMessageVel(self):
        joyCommand = Joy()
        trans, rot = self.tflistener.lookupTransform(
            "girona500", "world",
            self.tflistener.getLatestCommonTime("girona500", "world"))
        rotation_matrix = tf.transformations.quaternion_matrix(rot)
        command_vel = np.asarray([
            self.desVel[0],
            self.desVel[1],
            self.desVel[2], 1])
        command_vel_tf = np.dot(rotation_matrix, command_vel)[:3]
        joyCommand.axes.append(command_vel_tf[0])
        joyCommand.axes.append(command_vel_tf[1])
        joyCommand.axes.append(command_vel_tf[2])
...
if __name__ == '__main__':
    try:
        ...
        rospy.init_node('learning_reproductor_arm_vel')

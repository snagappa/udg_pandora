...
import rospy
#import tf
from lib import local_tf as tf
import PyKDL
import math
import code
import copy
...
from pose_ekf_slam.motion_transformations import linearVelocity
from geometry_msgs.msg import TwistWithCovarianceStamped

USE_COLA_TOPICS =  False
USE_GPS = False
USE_BUMBLEBEE = True

MSG_VALIDITY_TIME = rospy.Duration(0.005)

CAMERA_ROOT = "/stereo_camera"
...
class G500_SLAM():
    def __init__(self, name, nparticles=None):
        # Get config
        self.config = get_g500_config()
        
        # Main SLAM worker
        if not nparticles is None:
            self.slam_particles = nparticles
        else:
            self.slam_particles = 1
        self.sigma_mean = np.zeros(4)
        self.sigma_pts_mask = np.array([1, 1, 0, 1, 1, 0], dtype=np.bool)
        self.slam_worker = self.init_slam()
        self.image_landmarks = np.empty(0)
        """
        try:
        ...
        self.__LOCK__ = DummyLock() #threading.Lock()
    ...
    def init_slam(self):
        slam_worker = PHDSLAM(nparticles=self.slam_particles, resample_threshold=0.5)
    ...
    def init_config(self):
        ...
        #config.dvl_tf = self.compute_tf(
        #    np.array([-0.65, 0, 0.4, 0, 0, 225]))
    ...
    def init_ros(self, name):
        ...
        # TF broadcaster
        ros.tfbroadcaster = tf.TransformBroadcaster()
    ...
    def set_navigation(self, req):
        ...
        try:
            ...
            self.slam_worker.vehicle.states[:, :] = 0
        """
        self.slam_worker.set_states(
            states=np.asarray([[req.north, req.east,
                                self.vehicle.pose_position[2]]]))
        ret = SetNEResponse()
        ret.success = True
        return ret
    ...
    def update_dvl(self, dvl):
        ...
        if dvl_update != 0:
            ...
            #    return
            if not self.predict(config.last_time.dvl):
                self.ros.NO_LOCK_ACQUIRE += 1
                return
    ...
    def updateLinkquestDvl(self, dvl):
        config = self.config
        config.last_time.dvl = copy.copy(dvl.header.stamp)
        
        vel = TwistWithCovarianceStamped()
        vel.header.stamp = dvl.header.stamp
        vel.header.frame_id = dvl.header.frame_id
        measured_linear_velocity = STRUCT()
        if dvl.velocityInstFlag == 1:
            measured_linear_velocity.x = -dvl.velocityInst[0]
            measured_linear_velocity.y = -dvl.velocityInst[1]
            measured_linear_velocity.z = -dvl.velocityInst[2]
            #vel.twist.covariance[0] = 0.05
            #vel.twist.covariance[7] = 0.05
            #vel.twist.covariance[14] = 0.05
            self.vehicle.altitude = dvl.altitude
            mode = 'b'
        elif dvl.waterVelocityInstFlag == 1:
            measured_linear_velocity.x = -dvl.waterVelocityInst[0]
            measured_linear_velocity.y = -dvl.waterVelocityInst[1]
            measured_linear_velocity.z = -dvl.waterVelocityInst[2]
            #vel.twist.covariance[0] = 0.15
            #vel.twist.covariance[7] = 0.15
            #vel.twist.covariance[14] = 0.15
            self.vehicle.altitude = dvl.altitude
            mode = 'w'
        else:
        ...
        vehicle_angular_velocity = STRUCT()
        (vehicle_angular_velocity.x,
         vehicle_angular_velocity.y,
         vehicle_angular_velocity.z) = self.vehicle.twist_angular
        rot_sensor_from_vehicle = np.array([0, 0, 0, 1])
        self.vehicle.twist_linear = linearVelocity(measured_linear_velocity,
            vehicle_angular_velocity, self.config.dvl_tf_data[0:3],
    ...
    def update_svs(self, svs):
        #print os.getpid()
        config = self.config
        # Don't do anything unless initialised
        if not (config.init.dvl and config.init.imu):
            return
    ...
    def update_features(self, pcl_msg):
        ...
        self.__LOCK__.acquire()
        try:
            if pcl_msg.header.stamp == rospy.Time():
                self.config.last_time.map = copy.copy(self.config.last_time.predict)
            else:
                self.config.last_time.map = copy.copy(pcl_msg.header.stamp)
            pcl_msg.header.stamp = self.config.last_time.map
            ...
            #if 0.5 < self.vehicle.altitude < 5:
            #    slam_features[:, 2] += self.vehicle.altitude
            #    slam_features[:, 2] /= 2
            # We can now access the points as slam_features[i]
            self.slam_worker.update_features(slam_features)
            print "Finished updating features."
            # Get vehicle state with highest weight
            max_wt_idx = self.slam_worker.vehicle.weights.argmax()
            # get projection of map onto the image
            print "Requesting image points"
            self.image_landmarks = self.slam_worker.vehicle.maps[max_wt_idx].map_to_image_points()
            self.slam_worker.resample()
            """
            ...
            if eff_nparticles:# < 1.5:
                ...
                print "Moving states..."
                print "Time = ", self.config.last_time.map.to_sec()
    ...
    def predict(self, predict_to_time):
        ...
        if not config.init.init:
            ...
            pose_angle = tf.transformations.euler_from_quaternion(
                                                self.vehicle.pose_orientation)
            # Check for prediction time deliberately set to zero
            if predict_to_time == rospy.Time():
                return True
            # Else check if timestamp within acceptable window
            if predict_to_time < (config.last_time.predict - MSG_VALIDITY_TIME):
                self.ros.NO_LOCK_ACQUIRE += 1
                return False
    ...
    def publish_transforms(self, *args, **kwargs):
        ...
        br = self.ros.tfbroadcaster
        position = self.slam_worker.get_position()
        """
        br.sendTransform(position[0], 
                         platform_orientation, self.config.last_time.predict, 
        ...
        for particle_idx in range(nparticles):
            ...
            parent_state = position[particle_idx]
            if not np.any(np.isnan(parent_state)):
                ...
                br.sendTransform(t_sensor, o_sensor, timestamp, 
    ...
    def publish_data(self, *args, **kwargs):
        ...
        odom_msg.header.frame_id = self.ros.name
        
        odom_msg.pose.pose.position.x = est_state[0]
        odom_msg.pose.pose.position.y = est_state[1]
        odom_msg.pose.pose.position.z = est_state[2]
        odom_msg.pose.pose.orientation.x = self.vehicle.pose_orientation[0]
        odom_msg.pose.pose.orientation.y = self.vehicle.pose_orientation[1]
        odom_msg.pose.pose.orientation.z = self.vehicle.pose_orientation[2]
        odom_msg.pose.pose.orientation.w = self.vehicle.pose_orientation[3]
        odom_msg.pose.covariance[0] = est_cov[0, 0]
        odom_msg.pose.covariance[7] = est_cov[1, 1]
        odom_msg.pose.covariance[14] = est_cov[2, 2]
        odom_msg.twist.twist.linear.x = self.vehicle.twist_linear[0]
        odom_msg.twist.twist.linear.y = self.vehicle.twist_linear[1]
        odom_msg.twist.twist.linear.z = self.vehicle.twist_linear[2]
        odom_msg.twist.twist.angular.x = self.vehicle.twist_angular[0]
        odom_msg.twist.twist.angular.y = self.vehicle.twist_angular[1]
        odom_msg.twist.twist.angular.z = self.vehicle.twist_angular[2]
        self.ros.odom.publisher.publish(odom_msg)

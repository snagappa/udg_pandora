...
class VisualDetector(object):
    def __init__(self, name):
        ...
        if self.use_endeffector:
            ...
            print "Not using end effector camera"
    ...
    def init_panel_detector(self):
        ...
        if self.enable_detect_valves:
            ...
            # rospy.loginfo('!!!!!!!!!!!!!!!!!!! Valve centres ' + str(valves.centre[:, :2]))
        ...
        panel.pixel_matches_msg = Point2DMatched()
    ...
    def detect_panel(self, *args):
        ...
        if panel_detected:
            ...
            self.tf_broadcaster.sendTransform(tuple(panel_centre),
                panel_orientation_quaternion,
                time_now, "subpanel_centre", self._camera_.frame_id)
            ...
            print "Panel World Position: ", \
                np.squeeze(transform_numpy_array("world", "subpanel_centre",
                                                 np.zeros((1, 3))))
            print "Relative position   : ", panel_centre
            print "Standard deviation  : ", (
                panel_cov[range(3), range(3)]**0.5, "\n")
...
#            return
        ...
        #self.panel.detector.show()
        (panel_detected,
         panel_centre, panel_orientation, panel_cov) = detector.location()
        ...
        if panel_detected:
            valves = self.endeffector.valves
            # ARNAU
            template_px, scene_px = self.endeffector.detector.get_pixel_matches()
            if not template_px is None:
                _template_points2d_ = [Point2D(_tpl_px_[0], _tpl_px_[1])
                                       for _tpl_px_ in template_px]
                _scene_points2d_ = [Point2D(_tpl_px_[0], _tpl_px_[1])
                ...
                self.endeffector.pixel_matches_msg.header.stamp = time_now
                self.endeffector.pixel_matches_msg.img0 = _template_points2d_
                self.endeffector.pixel_matches_msg.img1 = _scene_points2d_
                self.endeffector.pixel_matches.publish(self.endeffector.pixel_matches_msg)
    ...
    def detect_valves(self, detection_msg):
        ...
        if (panel_detected and (np.linalg.norm(panel_centre) <= 2) and
            ...
            valve_msg.header.frame_id = "subpanel_centre"
            ...
            for valve_idx in range(valves.centre.shape[0]):
                ...
                if (left < 0 or top < 0 or
                    ...
                    rospy.loginfo('Imatge is outsite the limits')
                    rospy.loginfo('Left ' + str(left) + ' Right ' + str(right))
                    rospy.loginfo('Top ' + str(top) + ' Bottom ' + str(bottom))
                    rospy.loginfo('SceneX ' + str(scene.shape[1]) + ' SceneY ' + str(scene.shape[0]))
                else:
                    # Get approximate length of the valve in pixels
                    rospy.loginfo('Image is in the limits')
                    bbox_side_lengths = (
                        ((np.diff(np.vstack((px_corners, px_corners[0])),
                                  axis=0)**2).sum(axis=1))**0.5)
                    valve_length_px = np.int32(0.8*np.mean(bbox_side_lengths))
                    ...
                    if not this_valve_orientation is None:
                        ...
                        self.tf_broadcaster.sendTransform(
                            ...
                            "valve"+str(valve_idx), "subpanel_centre")
    ...
    def detect_valve_orientation(self, img, bbox=None,
        ...
        # rospy.loginfo('CannyThreshold2 : ' + str(CannyThreshold2))
        CannyThreshold1 = 10
        CannyThreshold2 = 250
        # Bluring the image
        ...
        if lines.ndim == 1:
            rospy.loginfo(' Una linia ')
            valve_points = lines
        elif lines.ndim == 2:
            rospy.loginfo(' Dues linies ')
            pts1 = lines[:, :2]
            pts2 = lines[:, 2:]
            ...
            rospy.loginfo(' Linies ' + str(lines.ndim))

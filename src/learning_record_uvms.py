...
class LearningRecord:
    ...
    def updateGoalPose(self, pose_msg):
        ...
        try:
            ...
            self.valveOri = np.array([pose_msg.pose.pose.orientation.x,
    ...
    def updateArmPose(self, armPose):
        ...
        try:
            if self.initGoalPose and self.initRoll:
                ...
                valve_ori_mat = tf.transformations.quaternion_matrix(self.valveOri)
                ...
                #Debug
                # rospy.loginfo('End-effector Ori ' + str(arm_ori))
                ...
                arm_world_ori = np.dot(trans_matrix_v2[0:3, 0:3],
                                       arm_ori[0:3, 0:3])
                #Debug
                # euler_arm_ori = tf.transformations.euler_from_matrix(
                #     arm_world_ori)
                # rospy.loginfo('End-effector World ' + str(euler_arm_ori))
                # euler_arm_ori = tf.transformations.euler_from_matrix(
                #     valve_ori_mat)
                # rospy.loginfo('Valve World ' + str(euler_arm_ori))
                # arm_frame_ori = np.dot(valve_ori_mat[0:3, 0:3], arm_world_ori)
                # euler_arm_ori = tf.transformations.euler_from_matrix(
                #     arm_frame_ori)
                # rospy.loginfo('Not Inverted World ' + str(euler_arm_ori))
                # #Panel ori
                # euler_arm_ori = tf.transformations.euler_from_matrix(
                #     trans_matrix)
                # rospy.loginfo('Panel Ori World ' + str(euler_arm_ori))
                # End Debug
                ...
                #Debug
                # rospy.loginfo('End-effector Valve ' + str(euler_arm_ori))
                # rospy.loginfo('****************************************')
                ...
                s = (repr(arm_frame_pose[0]) + " " +
                     ...
                     repr(euler_arm_ori[0]) + " " +
                     repr(euler_arm_ori[1]) + " " +
                     repr(euler_arm_ori[2]) + " " +
                     repr(rospy.get_time()) + "\n")
                self.file.write(s)
            else:

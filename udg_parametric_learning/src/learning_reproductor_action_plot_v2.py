...
#        pub = rospy.Publisher('arm', )
        ...
        dmp_y = LearningDmpParamReproductor(
            ...
            self.interval_time,
            nb_groups)
        dmp_x_yaw = LearningDmpParamReproductor(
        ...
        dmp_arm_z = LearningDmpParamReproductor(
            ...
            self.interval_time,
            nb_groups)
        rospy.loginfo('Learning parameters ' + str(self.reproductor_parameters[4]) + ' \nPath ' + str(path))
        dmp_arm_x_y_yaw = LearningDmpParamReproductor(
            self.reproductor_parameters[4],
            path,
            3,
            #1,
            self.alpha,
                ...
                if self.enabled:
                    if not self.simulation:
                        if self.dataReceived > 1 and self.dataReceivedArm > 1:
                            ...
                            [des_pose_y, des_vel_y] = dmp_y.generateNewPose(
                                self.currPos, self.currVel,
                                self.action, self.param)
                            [des_pose_x_yaw, des_vel_x_yaw] = dmp_x_yaw.generateNewPose(
                                self.currPos, self.currVel,
                                self.action, self.param)
            ...
            if not self.simulation:
                if self.dataReceived > 1 and self.dataReceivedArm > 1:
                    ...
                    if (len(des_pose_z) != 0 and len(des_pose_x_yaw) != 0
                        ...
                        self.desPos[4] = des_pose_arm_x_y_yaw[0]
                        self.desVel[4] = (des_vel_arm_x_y_yaw[0]
                                          - des_vel_x_yaw[0])
                        self.desPos[5] = des_pose_arm_x_y_yaw[1]
                        self.desVel[5] = (des_vel_arm_x_y_yaw[1]
                                          - des_vel_y)
                        self.desPos[6] = des_pose_arm_z

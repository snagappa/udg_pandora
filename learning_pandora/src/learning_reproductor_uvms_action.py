...
class learningReproductorAct:
    ...
    def __init__(self, name):
        ...
        self.pub_uvms = rospy.Publisher(
            "/cola2_control/uvms_vel_cmd", Twist)
        self.pub_visual_uvms = rospy.Publisher(
            "/visual_servo_uvms/eeVelCmds", Twist)
    ...
    def updateArmPosition(self, data):
        ...
        try:
            if self.dataGoalReceived and self.dataRollReceived :
                ...
                arm_ori = tf.transformations.quaternion_matrix([
                    data.pose.orientation.x,
                    data.pose.orientation.y,
                    data.pose.orientation.z,
                    data.pose.orientation.w])

                valve_ori_mat = tf.transformations.quaternion_matrix(
                    self.valveOri)
                inv_valve_ori_rot = np.transpose(valve_ori_mat[0:3, 0:3])


                arm_world_ori = np.dot(trans_matrix_v2[0:3, 0:3],
                                       arm_ori[0:3, 0:3])

                arm_frame_ori = np.dot(inv_valve_ori_rot, arm_world_ori)
                self.armOrientation = tf.transformations.euler_from_matrix(
                    arm_frame_ori)

                #self.armOrientation = euler_ar
                ...
                self.prevPos[0:6] = self.currPos[0:6]
                self.currPos[0:3] = self.armPose
                self.currPos[3:6] = self.armOrientation
                ...
                if self.dataReceivedArm == 0:
                    ...
                    self.currTimeArm = (data.header.stamp.secs +
                                        (data.header.stamp.nsecs*1E-9))
                    self.currVel[0:6] = ((self.currPos[0:6]-self.prevPos[0:6]) /
...
#        pub = rospy.Publisher('arm', )
    ...
    def valve_turning_act(self, goal):
        ...
        #Stop all the of the UVMS sending Zero velocities
        ee_vel = Twist()
        ee_vel.linear.x = 0.0
        ee_vel.linear.y = 0.0
        ee_vel.linear.z = 0.0
        ee_vel.angular.x = 0.0
        ee_vel.angular.y = 0.0
        ee_vel.angular.z = 0.0
        self.pub_uvms.publish(ee_vel)
    ...
    def generateNewPose(self):
        ...
        diff[4] = cola2_lib.normalizeAngle(diff[4])
        diff[5] = cola2_lib.normalizeAngle(diff[5])
    ...
    def publishCommands(self):
        ...
        trans_valve = tf.transformations.quaternion_matrix(
            self.valveOri)
        ...
        vel_ori_world = np.dot(trans_valve, vel_ori_panel)
        ...
        vel_ori_auv = np.dot(inv_mat_auv, vel_ori_world)

        ee_vel = Twist()
#ERORORO
        ee_vel.linear.x = vel_auv[0]
        ee_vel.linear.y = -vel_auv[1]
        ee_vel.linear.z = vel_auv[2]

        ee_vel.angular.x = 0.5*vel_ori_auv[0]
        ee_vel.angular.y = 0.5*vel_ori_auv[1]
        ee_vel.angular.z = 0.5*vel_ori_auv[2]
        self.pub_uvms.publish(ee_vel)

        # ee_vel = Twist()
        # ee_vel.linear.x = vel_world[0]
        # ee_vel.linear.y = -vel_world[1]
        # ee_vel.linear.z = vel_world[2]

        rospy.loginfo('Vel pose ' + str(vel_auv[0:3]))
        rospy.loginfo('Vel orientation '  + str(vel_ori_auv[0:3]) )
        # ee_vel.angular.x = vel_ori_world[0]
        # ee_vel.angular.y = -vel_ori_world[1] #-> ull aqui hi ha errors
        # ee_vel.angular.z = vel_ori_world[2]
        # self.pub_visual_uvms.publish(ee_vel)

        # rospy.loginfo('Curr Pos ' + str(self.currPos[0:3]) )
        # rospy.loginfo('Vel Valve ' + str(vel_world[0:3]))
        # rospy.loginfo('Vel World ' + str(vel_world[0:3]))
        # rospy.loginfo('Vel Frame AUV ' + str(vel_auv[0:3]))
        rospy.loginfo('****************************************************')
